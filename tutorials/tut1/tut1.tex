...
\begin{homeworkProblem}
    ...
    Domain of \(a\):\\
    \begin{itemize}
        \item \textit{Radians}: \([0, 2\pi]\)
        \item \textit{Degrees}: \([0^{\circ}, 360^{\circ}]\)
    \end{itemize}
...
\begin{homeworkProblem}
    ...
    \textbf{Solution}\\
    \textbf{Converting from Cartesian to Cylindrical Coordinates}\\
    At first glance, we can easily to derive \(h\) from y:
    \begin{center}
        \(h = y\)
    \end{center}
    Next, \(r\) and \(a\) can be derived from x and z like converting
    ...
    \textbf{Converting from Cartesian to Spherical Coordinates}\\
    For converting from cartesian coordinates to spherical coordinates, the
    co-ordinate \(r\) can be easily obtained using the following formula:
    \begin{center}
        \(r = \sqrt{x^{2} + y^{2} + z^{2}}\)\\
    \end{center}
    The co-ordinate \(\beta\) can be obtained similar to how it is obtained for
    the 2-dimensional polar co-ordinates:
    \begin{center}
        \(\beta = \tan^{-1}(y/x)\)\\
    \end{center}
    Finally, to derive the co-ordinate \(\alpha\) we look at the following figure:
    \begin{figure}[H]
        \includegraphics[width=6cm]{fig/q3_1.PNG}
        \centering
    \end{figure}
    Which allows us to arrive at the derivation:
    \begin{center}
        \(\alpha = \tan^{-1}(\sqrt{x^{2} + y^{2}}/z)\)\\
    \end{center}
